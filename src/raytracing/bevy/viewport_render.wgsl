...
fn traverse_brick(
    ray: ptr<function, Line>,
    ray_current_point: ptr<function,vec3f>,
    current_distance: ptr<function, f32>,
    brick_start_index: u32,
    brick_bounds: ptr<function, Cube>,
    ray_scale_factors: ptr<function, vec3f>,
    direction_lut_index: u32,
    max_distance: f32, // NOTE: This is now linear distance, not squared
) -> BrickHit {
    ...
    */// --- DEBUG ---
    ...
    loop{
        ...
        */// --- DEBUG ---
        ...
        if stage_data.stage == VHX_PREPASS_STAGE_ID && *current_distance >= max_distance
        {
            return BrickHit(false, vec3u(current_index), u32(current_flat_index));
        }

        let step_result = dda_step_to_next_sibling(
            ray, ray_current_point, &current_bounds, ray_scale_factors
        );
        step = round(step_result.xyz);
        *current_distance += step_result.w;
    ...
    if(( // node is occupied at target child_sectant, meaning: brick is not empty
        ...
        if(0 != (0x80000000 & brick_descriptor)) { // brick is solid
            ...
            return OctreeRayIntersection(
                ...
                color_palette[brick_descriptor & 0x0000FFFF],
                *ray_current_point,
                vec3f(0.,1.,0.)
            );
            ...
            let leaf_brick_hit = traverse_brick(
                ...
                current_distance, // Pass the distance pointer
    ...
    if(node_mips[node_key] != EMPTY_MARKER) { // there is a valid mip present
        if(0 != (node_mips[node_key] & 0x80000000)) { // MIP brick is solid
            ...
            return OctreeRayIntersection(
                ...
                color_palette[node_mips[node_key] & 0x0000FFFF],
                *ray_current_point,
                vec3f(0.,1.,0.)
            );
        } else { // brick is parted
...
fn get_by_ray(ray: ptr<function, Line>, start_distance: f32) -> OctreeRayIntersection {
    ...
    // OPTIMIZED: Use linear distance, no pow() or squaring needed.
    let max_distance = select(
        f32(boxtree_meta_data.boxtree_size) * 2.,
    ...
    // OPTIMIZED: Track linear distance traveled incrementally.
    var current_distance = start_distance;
    ...
    if (0. == start_distance && root_intersect.impact_hit == true) {
        ...
        current_distance += root_intersect.impact_distance;
    }
    ...
    while (target_sectant < BOX_NODE_CHILDREN_COUNT && current_distance < max_distance) {
        ...
        current_bounds.min_position = vec3f(0.);
        ...
        while (0 != (node_stack_meta & 0x000000FFu) && current_distance < max_distance) {
            ...
            let ocbox_min_point = current_bounds.min_position + node_ocbox_unit * vec3f(
                f32(node_occupied_box[current_node_key] & 0x1Fu),
                f32((node_occupied_box[current_node_key] >> 5u) & 0x1Fu),
                f32((node_occupied_box[current_node_key] >> 10u) & 0x1Fu)
            );
            let ocbox_max_point = current_bounds.min_position + node_ocbox_unit * vec3f(
                f32((node_occupied_box[current_node_key] >> 15u) & 0x1Fu),
                f32((node_occupied_box[current_node_key] >> 20u) & 0x1Fu),
                f32((node_occupied_box[current_node_key] >> 25u) & 0x1Fu)
            );
            ...
            if( target_sectant >= BOX_NODE_CHILDREN_COUNT
                || (0 != (current_node_metadata & 0x02u))
                || !ocbox_aabb_intersect(ray_current_point, ray_inv_dir, ocbox_min_point, ocbox_max_point)
                || (
                    0 == (RAY_TO_NODE_OCCUPANCY_BITMASK_LUT[target_sectant][direction_lut_index * 2] & current_occupied_bits[0])
                    && 0 == (RAY_TO_NODE_OCCUPANCY_BITMASK_LUT[target_sectant][direction_lut_index * 2 + 1] & current_occupied_bits[1])
                )
            ) {
                ...
                let step_result = dda_step_to_next_sibling(ray, &ray_current_point, &target_bounds, &ray_scale_factors);
                tmp_vec = round(step_result.xyz);
                current_distance += step_result.w;
            ...
            // PUSH Condition
            if ( (target_child_descriptor != EMPTY_MARKER)
        ...
        current_distance += 10.0 * VOXEL_EPSILON;
